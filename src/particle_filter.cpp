...
#define EPS 0.001  // sammles allowed yawrate
...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
	// calculate constants only once for better efficiency
	const double vel_dt = velocity * delta_t;
	const double yaw_dt = yaw_rate * delta_t;
	const double vel_to_yawrate = velocity/yaw_rate;
	static default_random_engine gen;
    gen.seed(321);
	...
	for (int i = 0; i < num_particles; i++){
        if (fabs(yaw_rate) < EPS){
            ...
            particles[i].x += vel_to_yawrate * (sin(theta_new) - sin(particles[i].theta));
            particles[i].y += vel_to_yawrate * (-cos(theta_new) + cos(particles[i].theta));
...
// Update the weights of each particle using a mult-variate Gaussian distribution. You can read
	...
	// NOTE: The observations are given in the VEHICLE'S coordinate system. The particles are initially located
	//   according to the MAP'S coordinate system. There is a transformation between the two systems.
	//   This transformation includes both rotation and translation (but no scaling).
